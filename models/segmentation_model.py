...
class DomainAdaptationSegmentationModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            if self.use_refign and not adapt_to_ref:
                ...
                if self.use_align:
                    ...
                    m_logits_ref = warped_m_logits_ref
                else:
                    m_probs_trg = self.refine(
        ...
        if self.use_refign and not adapt_to_ref:
            ...
            mixed_img, mixed_lbl, mixed_weight = self.get_dacs_mix2(
                images_trg, m_probs_trg, images_src, gt_src) 
            
            # Train on mixed images
            mixed_pred = self.head(self.backbone(mixed_img))
            # mixed_pred_ref = self.head(self.backbone(mixed_img_ref))
            if self.use_hrda:
    ...
    #     save_dir = '/home/featurize/data/Our/refign-main/test/'
    ...
    @torch.no_grad()
    def get_dacs_mix2(self, images_trg, probs_trg, images_src, gt_src):
        ...
        mixed_img, mixed_lbl = [None] * images_bs, [None] * images_bs
        #1 =  local_iter + 1
        mix_masks = get_class_masks2(gt_src.unsqueeze(1))
